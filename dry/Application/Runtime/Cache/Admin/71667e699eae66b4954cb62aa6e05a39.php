...
<div class="col-md-12 graphs">
	<div class="xs">
  	 <h3>管理员列表</h3>
	 <a href="/admin.php/Manager/form" class="btn fb1" style="color:white;">添加管理员</a>
			...
			<table class="table table-striped">
				...
				<tbody>
					<?php if(in_array(($_SESSION['uid']), explode(',',"1,2"))): if(is_array($manager_list)): foreach($manager_list as $key=>$manager): if(($manager["user_id"]) != "1"): ?><tr>
						...
						<td>
						  <a href="/admin.php/Manager/form/id/<?php echo ($manager[user_id]); ?>">编辑</a>
						  <a href="/admin.php/Manager/delete/id/<?php echo ($manager[user_id]); ?>" onclick="if (!confirm('是否确认删除？')) return false;">删除</a>
						</td>						
					...
					<?php if(!in_array(($_SESSION['uid']), explode(',',"1,2"))): if(is_array($manager_list)): foreach($manager_list as $key=>$manager): if(($manager["user_id"]) != "1"): ?><tr>
						...
						<td><?php echo ($manager[level]); ?></td>						
						<td>
						  <?php if(($manager["user_id"]) == $_SESSION['uid']): ?><a href="/admin.php/Manager/form/id/<?php echo ($manager[user_id]); ?>">编辑</a><?php endif; ?>
						  <!-- <a href="/admin.php/Manager/delete/id/<?php echo ($manager[user_id]); ?>" onclick="if (!confirm('是否确认删除？')) return false;">删除</a> -->
						</td>						
					</tr><?php endif; endforeach; endif; endif; ?>
				</tbody>
